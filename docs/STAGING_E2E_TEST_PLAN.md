...
9. Create a report definition/version, queue PDF generation, and persist generated artifact metadata through a mocked renderer/storage adapter.
...
|---|---|---|
| Email verification | Mock token is persisted and consumed; no Brevo credential is used | Brevo delivery remains blocked until owner rotation/revocation, then link routing, expiry, and replay behavior require real validation |
...
| OAuth | Provider adapters remain disabled; the journey uses the mocked verified-user boundary | Google/Microsoft consent, callback, account linking, and token lifecycle |
...
| Report | Real PostgreSQL queue/state/audit validation with mocked PDF renderer and object storage, plus separate local Excel/PDF smoke tests | Real artifact upload, download authorization, and visual acceptance |
| Payment provider | Stripe remains disabled and is covered only by its deterministic adapter tests | Stripe test-mode checkout, webhook signatures, invoices, retries, and entitlement synchronization |
