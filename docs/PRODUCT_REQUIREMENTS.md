...
| Carbon calculation ledger | Reviewed activity data, evidence/version/source locator, conversions, factor snapshot, formula, actor, timestamp, corrections, and reproducibility | Evidence-derived activity calculation, strict review/scope/unit gates, metering, immutable provenance, evidence linkage, search indexing, and read API implemented; approval/version supersession UI pending |
